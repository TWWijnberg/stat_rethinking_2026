...
- **dagitty** - DAG specification and causal analysis
- **tidyverse** - Data manipulation
- **here** - Portable path handling

## Workflow Learnings

These patterns emerged from completing A05/A06 and should guide future analyses:

### DAG-Driven Workflow
- **Always start with a DAG** - Define causal structure before writing any model code
- **Use `dagitty::adjustmentSets()`** to determine what to condition on
- **Mediators vs confounders matter:**
  - Total effect: condition on confounders only, NOT mediators
  - Direct effect: condition on mediators too
- **Colliders**: Never condition on colliders (opens backdoor paths)
- **Document causal reasoning** in `03_dag_analysis.R` before fitting models

### Synthetic Data Validation
- **Validate before real data** - Generate synthetic data with known parameters, fit model, check recovery
- **Parameter recovery check:** True values should fall within 89% CI
- **If model can't recover known parameters, something is wrong** - Fix before proceeding
- **Marginal vs conditional effects:** When NOT conditioning on mediators, intercepts absorb indirect effects
- **Same model spec** for synthetic and real analysis (ensures validation is meaningful)

### Code Organization Philosophy
- **Transparency over abstraction** - Keep analytical decisions visible in scripts
- **Minimal helpers** - Only extract functions when they genuinely reduce complexity
- **Inline model fitting** - Use `ulam()` directly so you can see exactly what's being fit
- **Functions YES:** Data prep, simulation, complex plotting
- **Functions NO:** Model fitting, effect calculations, interpretation (keep these visible)
- **Different purposes, different structure** - Synthetic analysis validates; real analysis interprets
