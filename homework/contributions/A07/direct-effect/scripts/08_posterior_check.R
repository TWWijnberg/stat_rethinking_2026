# A07 step 08 — Posterior check (direct-effect)
#
...
# ---- 4. Weight against GROUP SIZE -------------------------------------------
# This model conditions on Gs, so its fit along Gs matters too.
gbin <- cut(dat$Gs, quantile(dat$Gs, seq(0, 1, 0.25)), include.lowest = TRUE,
            labels = c("Q1 small", "Q2", "Q3", "Q4 large"))
gt <- do.call(rbind, lapply(levels(gbin), function(b) {
  i <- gbin == b; rm_ <- rowMeans(yrep[, i, drop = FALSE])
  data.frame(bin = b, n = sum(i),
             observed = sprintf("%.2f kg", to_kg(mean(y[i]))),
             predicted = sprintf("%.2f kg", to_kg(mean(rm_))),
             inside = ifelse(mean(y[i]) >= quantile(rm_, .055) &
                             mean(y[i]) <= quantile(rm_, .945), "yes", "NO"))
}))
cat("\nmean weight by group-size quartile:\n"); print(gt, row.names = FALSE)
...
# Gs held at its mean, so the line shows the relationship the estimand asks about
# rather than a curve that also wobbles with group size.
fgrid <- seq(min(dat$Fs), max(dat$Fs), length.out = 60)
ep <- posterior_epred(fit, newdata = data.frame(Fs = fgrid, Gs = 0))
